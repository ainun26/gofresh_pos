...
class Stock extends CI_Controller {
    ...
    public function stock_in_del() {
        ...
        if($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', 'Data Stock_In berhasil dihapus');
        }
        redirect('stock/in');
    }
    ...
    public function process() {
        ...
        if(isset($_POST['in_add'])) {
            ...
            if($this->item_m->check_stock($post['stock'], $post['id'])->num_rows() > 0) {
                $this->session->set_flashdata('error', "Qty melebihi stock item");
